...
const DEFAULT_QOL_SETTINGS: QolSettingsPayload = {
...
export function AdminSettingsPage() {
  ...
  const [qolSettings, setQolSettings] = useState<QolSettingsPayload>(DEFAULT_QOL_SETTINGS);
  const [qolMessage, setQolMessage] = useState<string | null>(null);
  const [qolError, setQolError] = useState<string | null>(null);
  ...
  const { data: qolSettingsData } = useQuery<QolSettingsPayload>({
    queryKey: ["admin-qol-settings"],
    queryFn: async () => {
      const response = await api.get<QolSettingsPayload>("/admin/qol-settings");
      return response.data;
    },
    enabled: isAdmin
  });
  ...
  const updateQolSettings = useMutation({
    mutationFn: async (payload: QolSettingsPayload) => {
      await api.put("/admin/qol-settings", payload);
    },
    onSuccess: async () => {
      setQolMessage("Paramètres QoL mis à jour.");
      setQolError(null);
      await queryClient.invalidateQueries({ queryKey: ["admin-qol-settings"] });
      await queryClient.invalidateQueries({ queryKey: ["qol-settings"] });
    },
    onError: () => {
      setQolMessage(null);
      setQolError("Impossible d'enregistrer les paramètres QoL.");
    }
  });
  ...
  const content = (
    <section className="space-y-6">
      ...
      <div className="rounded-lg border border-slate-800 bg-slate-900 p-4 space-y-4">
        <div>
          <h3 className="text-sm font-semibold text-white">Paramètres QoL</h3>
          <p className="text-xs text-slate-400">
            Ajustez l'affichage des dates, les notes et l'archivage automatique.
          </p>
        </div>
        {qolMessage ? (
          <div className="rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-200">
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            setQolMessage(null);
            setQolError(null);
            updateQolSettings.mutate(qolSettings);
          }}
        >
          <div className="grid gap-4 md:grid-cols-2">
            <label className="text-xs text-slate-300">
              Fuseau horaire
              <span title="Ex: Europe/Paris" className="ml-1 text-[11px] text-slate-500">
                ⓘ
              </span>
              <AppTextInput
                value={qolSettings.timezone}
                onChange={(event) =>
                  setQolSettings((prev) => ({ ...prev, timezone: event.target.value }))
                }
                className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100"
              />
            </label>
            <label className="text-xs text-slate-300">
              Format de date
              <span title="Ex: DD/MM/YYYY" className="ml-1 text-[11px] text-slate-500">
                ⓘ
              </span>
              <AppTextInput
                value={qolSettings.date_format}
                onChange={(event) =>
                  setQolSettings((prev) => ({ ...prev, date_format: event.target.value }))
                }
                className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100"
              />
            </label>
            <label className="text-xs text-slate-300">
              ...
              <span title="Laissez vide ou 0 pour désactiver." className="ml-1 text-[11px] text-slate-500">
                ⓘ
              </span>
              <AppTextInput
                type="number"
                min={0}
                value={qolSettings.auto_archive_days ?? 0}
                onChange={(event) =>
                  setQolSettings((prev) => ({
                    ...prev,
                    auto_archive_days: Number(event.target.value) || null
                  }))
                }
                className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100"
              />
            ...
            <label className="text-xs text-slate-300">
              Longueur max des notes
              <span title="Minimum recommandé 30 caractères." className="ml-1 text-[11px] text-slate-500">
                ⓘ
              </span>
              <AppTextInput
                type="number"
                min={30}
                value={qolSettings.note_preview_length}
                onChange={(event) =>
                  setQolSettings((prev) => ({
                    ...prev,
                    note_preview_length: Number(event.target.value)
                  }))
                }
                className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100"
              />
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="submit"
              className="rounded-md bg-indigo-500 px-4 py-2 text-xs font-semibold text-white hover:bg-indigo-400"
              disabled={updateQolSettings.isPending}
            >
              Enregistrer
            </button>
          </div>
