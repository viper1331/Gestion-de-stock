...
from backend.services.pdf_inventory_exports import export_pharmacy_inventory_pdf
...
async def export_pharmacy_inventory_pdf_endpoint(
    q: str | None = Query(default=None, description="Filtre nom/code-barres"),
    ...
    expiring_soon: bool = Query(default=False, description="Péremptions proches"),
    user: models.User = Depends(get_current_user),
) -> StreamingResponse:
    _require_permission(user, action="view")
    ...
    try:
        resolved = resolve_pdf_config("inventory_pharmacy")
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    pdf_bytes = export_pharmacy_inventory_pdf(site_key, user, filters)
